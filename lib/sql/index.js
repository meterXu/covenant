...
where a.name=? and b.path= ?`,
    ...
    responseList:`select * from response where path like ? and identifier like ? and collectionId = ? order by createTime desc`,
    ...
    editResponse:`update response set collectionId=?,identifier = ?,path=?,method=?,status=?,contentType=?,charset=?,headers=?,body=?,remark=? where id = ?`,
}
